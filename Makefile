...
ci-build-staging:
	...
	@[[ "${CI}X" = "X" ]] || docker build --build-arg BUILD_DATE="$(build_date)" --build-arg VERSION="${CI_COMMIT_REF_SLUG}.${CI_COMMIT_SHORT_SHA}" -t ${CI_PSONO_REGISTRY}/jfheinrich/pre-commit:${CI_COMMIT_REF_SLUG} .
